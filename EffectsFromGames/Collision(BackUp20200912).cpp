...
bool equel(float a, float b, float tolerance = 0.001f)
{
...
float correctMistake(float t, float r, const SimpleMath::Vector3 & worldDistance, float worldDistanceLength, const SimpleMath::Vector3 & origin, const SimpleMath::Vector3 & impactPoint)
{
	...
	if (impactPointRel.Dot(impactPointRel) < r*r)
	{
		...
		if (proj_r > 0)
		{
			t = t - (r - proj_r) / worldDistanceLength;
		}
		else
		{
			t = t - (r + proj_r + abs(proj_r)) / worldDistanceLength;
		}
	}
	return t < 0.0f ? 0.0f : t;
}
...
bool collisionWithEdge(const Cylinder& cylinder, const Line& line, const SimpleMath::Vector3 & worldDistance, SimpleMath::Vector3 & impactPoint, SimpleMath::Vector3 & impactNormal, float & impactT)
{
	...
	auto cylinder_rr = cylinder.r*cylinder.r;
	...
	//collide end spheres with line points (3d)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			for (int j = 0; j < 2; j++)
			{
				auto delta = vCylinderX[i] - vLineX[j];

				b = 2.0f*worldDistance.Dot(delta);
				c = delta.Dot(delta) - cylinder_rr;
		...
		if (rootFound)
		{
			...
			auto delta = lineX - newCylinderX;

			if (lastRootFoundOnIndex2 == 0)
			{
				collision = cylinderBA.Dot(delta) <= .0f;
			}
			else
			{
				collision = cylinderBA.Dot(delta) >= .0f;
	...
	SimpleMath::Vector2 pCylinderX = _origin;
	SimpleMath::Vector2 vpLineX[] = { _s1, _s2 };
	...
	if (!collision)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			auto delta = pCylinderX - vpLineX[i];

			b = 2.0f*_distance.Dot(delta);
			c = delta.Dot(delta) - cylinder_rr;
		...
		if (rootFound)
		{
			auto newCylinderA = cylinder.A + SimpleMath::Vector3::TransformNormal(t*distance, ToCylinderLocalSpace.Transpose());
	...
	if (!collision && _d.Length() != 0.0f)
	{
		...
		if (rootFound)
		{
			...
			auto cylinderX = cylinder.A + SkewLinesArgs.x * cylinderBA;
			...
			if (index != -1)
			{
				...
				if (abs(x_coord) <= faceHalfSize.x)
				{
					if (abs(y_coord) <= faceHalfSize.y)
					{
						...
						impactPoint = x_coord*faceBasis.Right() + y_coord*faceBasis.Up();
...
SimpleMath::Vector3 normalize(const SimpleMath::Vector3 & n)
{
	auto ret = n;
	ret.Normalize();
	return ret;
