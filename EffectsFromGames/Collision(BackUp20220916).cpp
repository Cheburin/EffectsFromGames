...
float correctMistake(float t, float r, const SimpleMath::Vector3 & worldDistance, float worldDistanceLength, const SimpleMath::Vector3 & origin, const SimpleMath::Vector3 & impactPoint)
{
	...
	if (impactPointRel.Dot(impactPointRel) < r*r)
	{
		...
		{
			t = t - (2.0f * abs(proj_r)) / worldDistanceLength;
		}
	}
	return t < 0.0f ? 0.0f : t;
}
...
bool collisionWithVertex(const Cylinder& cy, const SimpleMath::Vector3& ve, const SimpleMath::Vector3 & worldDistance, SimpleMath::Vector3 & impactPoint, SimpleMath::Vector3 & impactNormal, float & impactT)
{
	...
	//collide end spheres with line points (3d)
	{
		...
		if (rootFound)
		{
			auto newCylinderX = cy.vX[lastRootFoundOnIndex] + t*worldDistance;
			...
			if (collision)
			{
				t = correctMistake(t, cy.r, worldDistance, worldDistanceLength, newCylinderX, lineX);
				t = offsetBySmallDistantce(t, worldDistanceLength);
			...
			if (faceNormal.Dot(worldDistance) < .0f)
			{
				...
				if (!lastCollison)
				{
					...
					for (int l = 0; l < 2; l++)
					{
						...
						for (int k = 0; k < 2; k++)
						{
							lastCollison = collisionWithVertex(cylinder, lineX[k], worldDistance, lastImpactPoint, lastImpactNormal, last_t);
