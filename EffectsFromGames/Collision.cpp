...
bool collisionWithTriangle(const Cylinder & cylinder, const SimpleMath::Vector3& p0, const SimpleMath::Vector3& p1, const SimpleMath::Vector3& p2, const SimpleMath::Vector3 & worldDistance, SimpleMath::Vector3 & impactPoint, SimpleMath::Vector3 & impactNormal, float & impactT)
{
	impactT = 1.0f;
	...
	auto FaseNormal = Q1.Cross(Q2);
	FaseNormal.Normalize();

	SimpleMath::Vector4 pFace = vec4(FaseNormal, -FaseNormal.Dot(p0));
	float vFaceDotS[2] = { pFace.Dot(vec4(cylinder.A, 1.0f)), pFace.Dot(vec4(cylinder.B, 1.0f)) };
	...
	if (vFaceDotS[0] > cylinder.r && vFaceDotS[1] > cylinder.r)
	{
		...
		if (rootFound)
		{
			auto n0 = FaseNormal.Cross(Q1);
			auto n1 = FaseNormal.Cross(p2 - p1);
			auto n2 = FaseNormal.Cross(-Q2);
			...
			SimpleMath::Vector3 P = newCylinderX - cylinder.r * FaseNormal;

			bool IsNewCylinderXInsideIntoTriangle = n0.Dot(P - p0) > -0.00001 && n1.Dot(P - p1) > -0.00001 && n2.Dot(P - p2) > -0.00001;

			if (IsNewCylinderXInsideIntoTriangle)
			{
				collision = true;
...
bool collisionWithMesh(const Cylinder & cylinder, const SimpleMath::Matrix& meshBasis, const std::vector<SimpleMath::Vector3> &vertex, const SimpleMath::Vector3 & worldDistance, SimpleMath::Vector3 & impactPoint, SimpleMath::Vector3 & impactNormal, float & impactT)
{
	impactT = 1.0f;
	...
	for (int i = 0; i < vertex.size(); i += 3)
	{
		...
		SimpleMath::Vector3 p[] = { vertex[i], vertex[i + 1], vertex[i + 2] };

		//triangle
		if (collisionWithTriangle(cylinder, p[0], p[1], p[2], worldDistance, lastImpactPoint, lastImpactNormal, last_t))
		{
			if (last_t < _t)
			{
				lastCollison = true;
				...
				_collison = true;
				_impactPoint = lastImpactPoint;
				_impactNormal = lastImpactNormal;
				_t = last_t;
			}
		}
		...
		if (!lastCollison)
			for (int j = 0; j < 3; j++)
			{
				...
				Line line(A.x, A.y, A.z, B.x, B.y, B.z);
		...
		if (!lastCollison)
			for (int j = 0; j < 3; j++)
			{
				if (collisionWithVertex(cylinder, p[j], worldDistance, lastImpactPoint, lastImpactNormal, last_t))
				{
					if (last_t < _t)
					{
						lastCollison = true;
...
bool physCollisionWithMesh(const Capsule& ca, const Mesh& mesh, const SimpleMath::Vector3 & worldDistance, SimpleMath::Vector3 & impactPoint, SimpleMath::Vector3 & impactNormal, float & t)
{
	if (mesh.overlap)
		return false;
	...
	return collisionWithMesh(Cylinder(A.x, A.y, A.z, B.x, B.y, B.z, ca.r), mesh.getMatrix(), mesh.getVertex(), worldDistance, impactPoint, impactNormal, t);
}

void Debug();
extern char DebugBuffer[1024];

void HitInfo2::Push(std::vector<HitInfo2>* HitInfos, const HitInfo2& HitInfo)
{
	if (HitInfos)
	{
...
{
	float min_t = 1.0f, t;
	SimpleMath::Vector3 TheImpactPoint;
	SimpleMath::Vector3 TheImpactNormal;

	auto box_iter = GWorld.Boxes.begin();
	auto total_boxes = GWorld.Boxes.size();
	for (int i = 0; i < total_boxes; i++)
	{
		if (physCollisionWithBox(ca, box_iter->second, worldDistance, TheImpactPoint, TheImpactNormal, t))
		{
			if (t < min_t)
			{
				objectType = 1;
				objectName = box_iter->first;
				impactPoint = TheImpactPoint;
				impactNormal = TheImpactNormal;
				min_t = t;
			}
			HitInfo2::Push(HitInfos, { 1, box_iter->first, TheImpactPoint, TheImpactNormal, t });
		}
		box_iter++;
	}
	auto mesh_iter = GWorld.Meshes.begin();
	auto total_meshes = GWorld.Meshes.size();
	for (int i = 0; i < total_meshes; i++)
	{
		if (physCollisionWithMesh(ca, mesh_iter->second, worldDistance, TheImpactPoint, TheImpactNormal, t))
		{
			if (t < min_t)
			{
				objectType = 2;
				objectName = mesh_iter->first;
				impactPoint = TheImpactPoint;
				impactNormal = TheImpactNormal;
				min_t = t;
			}
			HitInfo2::Push(HitInfos, { 2, mesh_iter->first, TheImpactPoint, TheImpactNormal, t });
		}
		mesh_iter++;
	}
...
SimpleMath::Vector3 physGetSlidingVector(const SimpleMath::Vector3 & worldDistance, float remaining_t, SimpleMath::Vector3& impactNormal)
{
	auto perpWorldDir = worldDistance - 1.0001f * worldDistance.Dot(impactNormal) * impactNormal;
	perpWorldDir.Normalize();

	return remaining_t * worldDistance.Length() * perpWorldDir;
	//auto remainingWorldDistance = perpWorldDir;
}
//collision core
